...
class lilURL
{
	...
	/**
	 * alternative ID code for use with the IV4t site
	 * @param $length length of the code required (default 4 characters)
	 */
	function get_new_id($length = 4){
		$newid = $this->createRandomPassword($length);
		while($this->get_url($newid) != -1){
			$newid = $this->createRandomPassword($length);
		}
		return $newid;
	}
